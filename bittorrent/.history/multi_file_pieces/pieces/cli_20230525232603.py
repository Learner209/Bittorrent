...
def main():
    ...
    logging.debug("The optimistic-unchoking strategy: {}\n \
                  ...
                  .format("On" if args.optimistic_unchoking else "Off",
                          "On" if args.anti_snubbing else "Off",
                          "On" if args.choking_strategy else "Off",
                          "On" if args.end_game_mode else "Off",
                          "On" if args.rarest_piece_first else "Off"))

    loop = asyncio.get_event_loop()

    client = TorrentClient(
                torrent = Torrent(args.torrent),
                enable_optimistic_unchoking = True if args.optimistic_unchoking else False,
                enable_anti_snubbing = True if args.anti_snubbing else False,
                enable_choking_strategy = True if args.choking_strategy else False,
                enable_end_game_mode = True if args.end_game_mode else False,
                enable_rarest_piece_first = True if args.rarest_piece_first else False
                )
