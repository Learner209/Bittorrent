...
class Client():
    ...
    def request_for_connection(self, nat_type_id=0):
        ...
        ##print(type(self.pool), type(nat_type_id), type(self.master))
        self.sockfd.sendto((self.pool + ' {0}'.format(nat_type_id)).encode(), self.master)
        data, addr = self.sockfd.recvfrom(len(self.pool) + 3) ## this 3 length means "ok "
        data = data.decode()
        ...
        data, addr = self.sockfd.recvfrom(8) ## Corresponds to the server sending confiramtion messages both direcitons
    ...
    def recv_msg(self, sock, is_restrict=False, event=None):
        if is_restrict: ### client self is a restrict NAT
            while True:
