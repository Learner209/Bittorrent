#!/usr/bin/env python
# coding:utf-8
# reference https://github.com/laike9m/PyPunchP2P.git
import socket
import struct
import sys
...
def main():
    ...
    poolqueue = {} ## Pool queue buffers the known clients waiting be to be established connection
    ...
    while True:
        ...
        if data.startswith("msg "):
            # forward symmetric chat msg, act as TURN server to traverse symmetric NAT
            try:
            ...
            pool, nat_type_id = data.strip().split() ## Pool is the identity self-set by the client
            ...
            print("request received for pool: {0}".format(pool))
            ...
            if pool in symmetric_chat_clients: ## Only focused on symmetric_chat_clients scenario
                ...
                symmetric_chat_clients[pool] = (nat_type_id, addr) ## Add symmertric__chat__client first
